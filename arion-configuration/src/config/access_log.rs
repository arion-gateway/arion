...
use arion_data_plane_api::envoy_data_plane_api::envoy::config::core::v3::SubstitutionFormatString as EnvoySubstitutionFormatString;

use arion_data_plane_api::envoy_data_plane_api::envoy::config::core::v3::substitution_format_string::Format;
...
impl TryFrom<EnvoyAccessLog> for AccessLog {
    ...
    fn try_from(value: EnvoyAccessLog) -> Result<Self, Self::Error> {
        ...
        let logger = match fmt.flatten() {
            Some(SubstitutionFormatString { format, omit_empty_values }) => arion_format::LogFormatter::try_new(
                format.as_ref().map(AsRef::as_ref).unwrap_or(DEFAULT_ACCESS_LOG_FORMAT),
                omit_empty_values,
            ),
            None => arion_format::LogFormatter::try_new(DEFAULT_ACCESS_LOG_FORMAT, false),
        }
