# Arion WebAssembly SDK

The **Arion WebAssembly SDK** provides a high-level, idiomatic Rust API for writing WebAssembly plugins that run inside the **Arion Proxy** via the Wasmtime runtime.
...
- [Proc-Macro & FFI Code Generation (`#[arion_plugin]`)](#proc-macro--ffi-code-generation-arion_plugin)
...
The Arion WebAssembly ecosystem is divided into three focused crates:

```mermaid
graph TD
    A["arion-wasm-sdk<br/>(Guest SDK Facade)"] --> B["arion-wasm-sdk-macros<br/>(proc-macro #[arion_plugin])"]
    A --> C["arion-wasm-types<br/>(Shared ABI & Types)"]
    D["Arion Proxy Host<br/>(Wasmtime Engine)"] <== "Wasm FFI / ABI (bincode)" ==> A
```

1. **`arion-wasm-sdk`**: The primary guest-side SDK crate. It provides safe wrappers around hostcalls, context handles (`RequestHandle`, `ResponseHandle`), tracing subscribers, and shared memory primitives.
2. **`arion-wasm-sdk-macros`**: Contains the `#[arion_plugin]` proc-macro. It inspects your `Plugin` trait implementation and selectively emits `#[no_mangle] pub extern "C"` ABI functions for only the hooks you override.
3. **`arion-wasm-types`**: The standalone ABI crate shared between host and guest. It defines binary types (`CalloutRequest`, `GrpcCalloutRequest`, `DownstreamMetadata`, `HeaderMutation`, `FilterAction`, `ArionWasmError`, etc.) serialized via `bincode_next` across the host-guest boundary.
...
Add `arion-wasm-sdk` and `arion-wasm-types` to your Wasm plugin's `Cargo.toml`:
...
[dependencies]
arion-wasm-sdk = { path = "../arion-wasm-sdk" }
arion-wasm-types = { path = "../arion-wasm-types" }
http = "1.0"
tracing = "0.1"
```
...
The resulting `.wasm` binary in `target/wasm32-unknown-unknown/release/my_arion_filter.wasm` is ready to be loaded by Arion Proxy.
...
## Proc-Macro & FFI Code Generation (`#[arion_plugin]`)

The `#[arion_plugin]` attribute proc-macro automates FFI code generation.
...
sequenceDiagram
    autonumber
    participant Host as Arion Proxy Host
...
- **`get_header(name: &str) -> Result<Option<HeaderValue>, ArionWasmError>`**  
  Look up a single header by name.
- **`set_header(name: HeaderName, value: HeaderValue) -> Result<(), ArionWasmError>`**  
  Overwrites any existing header with `name`.
- **`add_header(name: HeaderName, value: HeaderValue) -> Result<(), ArionWasmError>`**  
  Appends a header value (useful for multi-valued headers like `Set-Cookie`).
- **`remove_header(name: &HeaderName) -> Result<(), ArionWasmError>`**  
  Deletes all headers matching `name`.
- **`replace_header(name: HeaderName, value: HeaderValue) -> Result<(), ArionWasmError>`**  
  Replaces existing header instances.
- **`get_headers_map() -> Result<HeaderMap, ArionWasmError>`**  
  Retrieves all current HTTP headers into a standard `http::HeaderMap`.
- **`set_headers_map(headers: &HeaderMap) -> Result<(), ArionWasmError>`**  
  Replaces all headers with the given `HeaderMap`.
- **`apply_header_mutations(mutations: &[HeaderMutation]) -> Result<(), ArionWasmError>`**  
  Executes a batch of header mutations in a single FFI hostcall for optimal performance.

### URI & Status API

- **`get_uri() -> Result<http::Uri, ArionWasmError>`**  
  *(Available on `RequestHandle`)* Retrieves the current request URI.
- **`set_uri(uri: &http::Uri) -> Result<(), ArionWasmError>`**  
  *(Available on `RequestHandle`)* Updates the request URI.
- **`get_status_code() -> Result<Option<http::StatusCode>, ArionWasmError>`**  
  *(Available on `ResponseHandle`)* Retrieves the current response status code.
- **`set_status_code(status: http::StatusCode) -> Result<(), ArionWasmError>`**  
...
- **`get_body() -> Result<bytes::Bytes, ArionWasmError>`**  
  Retrieves the buffered body payload as raw bytes.
- **`set_body(body: &[u8]) -> Result<(), ArionWasmError>`**  
...
- **`take_request() -> Result<http::Request<bytes::Bytes>, ArionWasmError>`**  
  Materializes the full HTTP request (method, URI, version, headers, and body).
- **`replace_request(req: &http::Request<bytes::Bytes>) -> Result<(), ArionWasmError>`**  
  Replaces the entire HTTP request with the provided object.
- **`take_response() -> Result<http::Response<bytes::Bytes>, ArionWasmError>`**  
  Materializes the full HTTP response (status, version, headers, and body).
- **`replace_response(res: &http::Response<bytes::Bytes>) -> Result<(), ArionWasmError>`**  
...
- **`schedule_direct_response(response: http::Response<bytes::Bytes>) -> Result<(), ArionWasmError>`**  
...
```rust
use arion_wasm_types::{DownstreamMetadata, DownstreamConnectionMetadata};
...
fn on_plugin_start(&mut self) {
    if let Ok(Some(config_str)) = arion_wasm_sdk::get_plugin_config() {
...
```rust
use arion_wasm_sdk::dispatch_http_call;
...
```rust
use arion_wasm_sdk::dispatch_grpc_call;
use arion_wasm_types::{GrpcCalloutRequest, GrpcCalloutResponse};
...
The `sleep(duration)` function suspends the execution of the WebAssembly module for the requested `Duration`. Because Arion utilizes an asynchronous Wasm runtime engine, `sleep` yields execution back to the host event loop and **does not block** proxy server threads or concurrent HTTP traffic:

```rust
use std::time::Duration;
use arion_wasm_sdk::sleep;
...
The `set_io_timeout(duration)` function sets an absolute execution deadline for subsequent asynchronous out-of-band I/O operations (such as `dispatch_http_call` or `dispatch_grpc_call`). If the external service fails to respond within the allocated deadline, the host aborts the operation and returns `Err(ArionWasmError::Timeout)`.

Call `clear_io_timeout()` after the I/O operation finishes to disarm the deadline:

```rust
use std::time::Duration;
use arion_wasm_sdk::{set_io_timeout, clear_io_timeout, dispatch_http_call, ArionWasmError};
use arion_wasm_types::CalloutRequest;
use smol_str::SmolStr;
...
arion_wasm_sdk::set_custom_metrics([
    ("requests_authenticated", "1"),
    ("auth_latency_ms", "12"),
])?;

// Inject access log variables
arion_wasm_sdk::set_access_log_operators([
    ("user_id", "user_12345"),
    ("auth_tier", "premium"),
])?;
```
...
Wasm module instances in Arion are isolated per thread/connection. To share state across concurrent requests and worker threads safely, the SDK provides host-backed **Shared Memory Primitives** under `arion_wasm_sdk::shared`.
...
```rust
use arion_wasm_sdk::prelude::*;
use arion_wasm_sdk::shared::SharedAtomicU64;
use arion_wasm_types::HeaderMutation;
...
impl Plugin for GlobalCounterFilter {
    fn on_plugin_start(&mut self) {
        let _ = arion_wasm_sdk::init_tracing();
...
```rust
use arion_wasm_sdk::prelude::*;
use arion_wasm_sdk::shared::SharedBlob;
...
use arion_wasm_sdk::prelude::*;
use arion_wasm_sdk::dispatch_grpc_call;
use arion_wasm_types::GrpcCalloutRequest;
use smol_str::SmolStr;
...
The SDK bridges standard Rust `tracing` events directly to Arion's host logging framework over FFI.
...
Functions return `Result<T, ArionWasmError>`. The `ArionWasmError` enum is part of the stable ABI:
...
- **Header / trailer hostcalls** take an `is_trailer: u32` flag (`0` = headers, `1` = trailers): `arion_get_header`, `arion_set_header`, `arion_get_headers_map`, `arion_apply_header_mutations`, etc.
- **Body hostcalls do not** take `is_trailer`:
  - `arion_get_body(body_ptr, max_len, written_len_ptr)`
  - `arion_set_body(body_ptr, body_len)`
...
- **Guest must export `arion_malloc(size) -> *mut u8`** (provided by this SDK in `lib.rs`). The host uses it when writing callout responses and downstream metadata into guest linear memory.
