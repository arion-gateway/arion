...
//! Example using the Arion Wasm SDK to perform a gRPC Callout
use arion_wasm_sdk::{
    bytes, dispatch_grpc_call, http, init_tracing, arion_plugin, FilterAction, HttpHeaders, Plugin, RequestHandle,
};
...
pub mod test_service {
    include!(concat!(env!("OUT_DIR"), "/arion.test.rs"));
}
...
impl Plugin for GrpcCalloutFilter {
    ...
    fn on_request_headers(&mut self, ctx: &RequestHandle<HttpHeaders>) -> FilterAction {
        ...
        let callout_req = GrpcCalloutRequest {
            ...
            service_name: SmolStr::new("arion.test.TestService"),
