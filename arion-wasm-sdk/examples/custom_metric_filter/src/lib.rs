...
//! Example using the Arion Wasm SDK to evaluate a custom metric.
//!
//! This plugin demonstrates how to set a custom metric dynamically
//! from within a Wasm plugin using `set_custom_metric`.

use arion_wasm_sdk::{
    init_tracing, arion_plugin, set_custom_metrics, FilterAction, HttpHeaders, Plugin, RequestHandle,
};
...
impl Plugin for CustomMetricFilter {
    ...
    fn on_request_headers(&mut self, _ctx: &RequestHandle<HttpHeaders>) -> FilterAction {
        ...
        match arion_wasm_sdk::set_custom_metrics(multi_metrics) {
