...
pub fn run() -> bool {
    ...
    let result = (|| -> Result<()> {
        ...
        proxy::run_arion(bootstrap, metrics, access_log_config, timezone)?;
        Ok(())
    })();

    if let Err(e) = result {
        tracing::error!("Arion proxy terminated with error: {e:?}");
        return false;
    }
    true
}
...
mod proxy_tracing {
    ...
    impl fmt::time::FormatTime for CustomTime {
        fn format_time(&self, w: &mut fmt::format::Writer<'_>) -> std::fmt::Result {
            let offset_sec = arion_lib::timezone::local_offset_sec().unwrap_or(0);
            let tz =
