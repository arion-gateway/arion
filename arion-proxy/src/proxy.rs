...
#[cfg(feature = "tracing")]
use {
    arion_configuration::config::network_filters::tracing::{TracingConfig, TracingKey},
...
struct ServiceInfo {
    ...
    listener_factories: Vec<arion_lib::ListenerFactory>,
    clusters: Vec<arion_lib::PartialClusterType>,
...
fn launch_runtimes(
    ...
    let tracing = bootstrap
        ...
        .listeners
        .iter()
        .flat_map(arion_configuration::config::Listener::get_tracing_configurations)
        .collect::<HashMap<_, _>>();
    ...
    // - The bootstrap loader from arion-data-plane-api gets the list of cluster names used
    ...
    {
        arion_lib::instrumentation::dump_instrumentation_counters()
    }
...
async fn spawn_services(info: ServiceInfo) -> Result<()> {
    ...
    set.spawn(async move {
        ...
        let mcp_handler = xds_connect
            .as_ref()
            .map(|(_, sub_mgr, _)| arion_lib::mcp_xds_handler::init_mcp_xds_handler(StdArc::clone(sub_mgr)));
    ...
    if let Some(conf) = access_log_config {
        ...
        set.spawn(async move {
            ...
            for (target, access_log_config) in listeners
                .iter()
                .flat_map(arion_configuration::config::Listener::all_access_log_configs)
                .collect::<Vec<_>>()
            {
    ...
    if let Some(tz) = timezone {
        ...
        arion_lib::timezone::init_tz_cache(&mut set, &tz)?;
    }
    ...
    if tracing.is_empty() {
        ...
        arion_tracing::otel_update_tracers(tracing)?;
    }
...
async fn push_initial_listeners(
    bootstrap: Bootstrap,
    listeners: Vec<arion_lib::ListenerFactory>,
    configuration_senders: Vec<ConfigurationSenders>,
) -> Result<()> {
...
async fn start_proxy(configuration_receivers: ConfigurationReceivers) -> Result<()> {
    arion_lib::start_listener_manager(configuration_receivers).await?;
    Ok(())
}
