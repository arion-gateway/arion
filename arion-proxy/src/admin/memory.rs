...
pub async fn memory_handler(State(mut _admin_state): State<AdminState>) -> Json<Value> {
    ...
    let memory_heap_size = arion_stats::get_memory_heap_size().map_or(memory_physical, |v| v as u64);
    let memory_allocated = arion_stats::get_memory_allocated().map_or(memory_physical, |v| v as u64);
    Json(json!({
