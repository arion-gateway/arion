...
use arion_lib::clusters::clusters_manager::get_all_clusters;
use axum::{
    extract::State,
...
// Arion only tracks circuit breaker thresholds per priority; the last matching entry wins,
// mirroring `ClusterCircuitBreaker::from` in arion-lib.
...
fn write_cluster(out: &mut String, cluster: &ClusterConfig) -> fmt::Result {
    ...
    for (label, priority) in [("default_priority", RoutingPriority::Default), ("high_priority", RoutingPriority::High)]
    {
        ...
        // Arion has no separate max_pending_requests setting; Envoy's own default is 1024.
    ...
    // Arion does not currently distinguish clusters added via CDS from statically configured ones.
    ...
    for locality in localities {
        for endpoint in &locality.lb_endpoints {
            ...
            // Unhealthy is only ever set by active health checks in Arion today.
            ...
            // Arion does not implement outlier detection, so there is no success rate to report.
