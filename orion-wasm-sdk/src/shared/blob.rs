...
impl SharedBlob {
    ...
    pub fn read(&self) -> BlobData {
        ...
        loop {
            let size =
                unsafe { ffi::ext_shared_blob_read(self.id, buffer.as_mut_ptr(), buffer.len() as u32, &mut version) };
            if size <= buffer.len() as u32 {
                buffer.truncate(size as usize);
                return BlobData { data: buffer, version };
            }
    ...
    pub fn compare_and_swap(&self, data: &[u8], expected_version: u64) -> Result<u64, ()> {
        ...
        let new_ver = unsafe {
            ffi::ext_shared_blob_cas(self.id, data.as_ptr(), data.len() as u32, expected_version, &mut success)
        };
        if success == 1 {
            Ok(new_ver)
        } else {
