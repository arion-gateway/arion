...
impl<'a> RequestHandler<Request<ArionRequestBody>, UpstreamCallOpts<'a>> for &HttpChannels {
...
impl HttpChannel {
    ...
    async fn send_with_policy<F, Fut>(
        &self,
        mut req: Request<ArionRequestBody>,
        timeout: Option<Duration>,
        retry_policy: Option<&RetryPolicy>,
        priority: RoutingPriority,
        send: F,
    ...
    where
        F: Fn(Request<ArionRequestBody>) -> Fut,
        Fut: Future<Output = Result<Response<ArionResponseBody>>>,
    {
        ...
        Fut: Future<Output = Result<Response<ArionResponseBody>>>,
    {
        ...
        let mut last_result: Option<Result<Response<ArionResponseBody>>> = None;
        ...
        for (index, back_off) in retry_policy.exponential_back_off().iter().enumerate() {
            ...
            let cloned_req: Request<ArionRequestBody> = Request::from_parts(current_parts, cloned_body);

            // actually send the request and wait for the response...
            let result: Result<Response<ArionResponseBody>> = if let Some(t) = retry_policy.per_try_timeout() {
    ...
    fn map_upstream_result(
        result: Result<Response<ArionResponseBody>>,
        elapsed: Duration,
        route_timeout: Option<Duration>,
        version: http::Version,
    ) -> Result<hyper::Response<ArionResponseBody>> {
...
#[inline]
fn prepare_http1_request(request: &mut Request<ArionRequestBody>) -> Result<()> {
...
#[inline]
fn prepare_http2_request(request: &mut Request<ArionRequestBody>, is_tls: bool) -> Result<()> {
    // 1. Version rewrite
