...
impl Http1PoolInner {
    ...
    #[inline]
    pub fn pop(&self) -> Option<SendRequest<ArionRequestBody>> {
        self.idle.lock().pop().map(|conn| conn.tx)
    }

    #[inline]
    pub fn release(&self, tx: SendRequest<ArionRequestBody>) {
        if tx.is_closed() {
            return;
        }
...
impl Http1Permit {
    pub fn new(inner: StdArc<Http1PoolInner>, tx: SendRequest<ArionRequestBody>) -> Self {
        Self { inner, tx }
    }
...
impl Http1Pool {
    ...
    async fn checkout(&self) -> Result<SendRequest<ArionRequestBody>> {
        loop {
    ...
    #[inline]
    fn pop_idle(&self) -> Option<SendRequest<ArionRequestBody>> {
        self.inner.pop()
    }
...
async fn handshake<T>(io: T) -> Result<SendRequest<ArionRequestBody>>
where
