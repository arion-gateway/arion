...
impl<'a> TryFrom<(&'a RoutingRequirement, &'a Request<ArionRequestBody>, HashState<'a>)> for RoutingContext<'a> {
    type Error = RoutingContextError;

    fn try_from(
        value: (&'a RoutingRequirement, &'a Request<ArionRequestBody>, HashState<'a>),
