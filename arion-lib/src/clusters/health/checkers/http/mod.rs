...
where
    ...
    H: Send + 'static,
    for<'a> &'a H: RequestHandler<Request<ArionRequestBody>, UpstreamCallOpts<'a>>,
{
    tracing::debug!(
...
where
    ...
    for<'a> &'a H: RequestHandler<Request<ArionRequestBody>, UpstreamCallOpts<'a>>,
{
    type Response = Response<ArionResponseBody>;
...
fn create_request(
    ...
    host: &str,
    uri: &http::Uri,
) -> Result<Request<ArionRequestBody>, http::Error> {
    ...
    let req = req.header("User-Agent", "arion/health-checks");
