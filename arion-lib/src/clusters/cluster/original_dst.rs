...
use arion_http_header::X_ENVOY_ORIGINAL_DST_HOST;
...
impl OriginalDstCluster {
    fn apply_port_override<'a>(&self, authority: &'a Authority) -> Result<Cow<'a, Authority>> {
        match self.global.upstream_port_override {
            ...
            None => Ok(Cow::Borrowed(authority)),
        }
    }
    ...
    pub fn get_grpc_connection_by_authority(&mut self, authority: &Authority) -> Result<GrpcService> {
        ...
        let endpoint_addr = EndpointAddress::ref_cast(authority.as_ref());
        ...
        self.endpoints.insert(endpoint_addr.clone(), endpoint);
        Ok(grpc_service)
    }
...
impl Borrow<Authority> for EndpointAddress {
    fn borrow(&self) -> &Authority {
        &self.0
    }
}
...
mod tests {
    ...
    fn test_http_connection_by_authority() {
        ...
        let channel1 = cluster.get_http_connection_by_authority(&authority).unwrap();
    ...
    fn circuit_breaker_is_present_when_configured() {
        ...
        let state = cb.get_state(arion_configuration::config::cluster::RoutingPriority::Default);
    ...
    fn circuit_breaker_is_shared_across_clones() {
        ...
        cb.try_increment_connections(arion_configuration::config::cluster::RoutingPriority::Default).unwrap();

        let clone_cb = clone.circuit_breaker().unwrap();
        assert_eq!(
            clone_cb
                .try_increment_connections(arion_configuration::config::cluster::RoutingPriority::Default)
                .unwrap_err(),
