...
// Extended External Processor Configuration, specific to Arion
#[derive(Debug, Clone)]
...
impl ExternalProcessor {
    ...
    pub async fn apply_request_prepare_processing_data(
        &mut self,
        request: &mut Request<ArionRequestBody>,
    ...
    pub async fn apply_response_prepare_processing_data(
        &mut self,
        response: &mut Response<ArionResponseBody>,
    ) -> Result<ProcessingData, FilterDecision> {
...
impl<S: kind::Mode + Default> ExternalProcessingWorker<S> {
    ...
    fn build_direct_response(&mut self, response_attempt: &mut ImmediateResponse) -> Response<ArionResponseBody> {
