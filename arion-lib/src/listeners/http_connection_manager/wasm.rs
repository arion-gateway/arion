...
impl WasmFilter {
    ...
    async fn get_state(&mut self) -> Result<&mut WasmFilterState, WasmError> {
        ...
        if state_opt.is_none() {
            ...
            let state = if let Some(s) = self.inner.instance_pool.pop() {
                ...
                store.data_mut().arion_malloc = instance.get_func(&mut store, "arion_malloc");
    ...
    pub async fn apply_request(&mut self, req: &mut Request<ArionRequestBody>, req_ctx: &RequestCtx) -> FilterDecision {
