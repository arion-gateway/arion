...
async fn build_request_from_mock(mock_request: &MockMessage<RequestMsg>) -> Request<ArionRequestBody> {
...
async fn build_response_from_mock(mock_response: &MockMessage<ResponseMsg>) -> Response<ArionResponseBody> {
            ...
            BODY_MODIFICATIONS.iter().flat_map(move |&body| {
                TRAILER_MODIFICATIONS.iter().map(move |&trailers| {
                    create_headers_response::<M>(
                        vec![headers],
                        body.map(Into::into),
                        // trailers modifications are NYI in body response in Envoy but Arion supports them
                        vec![trailers],
                        status as i32,
                        None,
                    )
                ...
                match response {
                    ...
                    | ProcessingResponseType::ResponseBody(BodyResponse {
                        ...
                        if matches!(
                            ...
                            // Arion can perform header mutations on body response only in BodyProcessingMode::Buffered
...
use arion_configuration::config::core::{StringMatcher, StringMatcherPattern};
...
async fn test_header_append_action_append_if_exists_or_add() {
    ...
    let response = ProcessingResponseType::RequestHeaders(
        arion_data_plane_api::envoy_data_plane_api::envoy::service::ext_proc::v3::HeadersResponse {
            response: Some(CommonResponse {
...
async fn test_immediate_response_with_grpc_status() {
    let immediate_response = ImmediateResponse {
        ...
        grpc_status: Some(arion_data_plane_api::envoy_data_plane_api::envoy::service::ext_proc::v3::GrpcStatus {
