...
pub enum FilterDecision {
    ...
    Continue,
    Reroute,
    DirectResponse(Box<Response<ArionResponseBody>>),
}
...
(e.g. `cargo build -p arion-proxy --features wasm`)"
                    .into());
            },
...
impl HttpFilterValue {
    pub async fn apply_request(&mut self, request: &mut Request<ArionRequestBody>, ctx: &RequestCtx) -> FilterDecision {
        match self {
