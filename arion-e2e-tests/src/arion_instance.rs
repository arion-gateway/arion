...
impl ArionInstance {
    ...
    pub async fn spawn_auto_port(
        ...
        info!(?config_path, ?listener_name, "Spawning Arion instance with auto port discovery");

        let arion_bin = find_arion_binary()?;
        debug!(?arion_bin, "Found Arion binary");

        let mut cmd = Command::new(&arion_bin);
        ...
        let mut process = cmd.spawn().map_err(|e| {
            Error::ProcessStartFailed(format!("Failed to spawn arion binary at {}: {e}", arion_bin.display()))
        })?;
            ...
            std::thread::spawn(move || {
                ...
                for line in reader.lines() {
                    ...
                    if let Ok(line) = line {
                        ...
                        if verbose {
                            eprintln!("[ARION-ERR] {line}");
                        }
                        debug!(target: "arion_stderr", "{}", line);
                    }
                }
            })
        });
                ...
                for line in reader.lines() {
                    ...
                    match line {
                        Ok(line) => {
                            ...
                            debug!(target: "arion_output", "{}", line);
                        },
                        Err(e) => {
                            warn!("Error reading arion output: {}", e);
                            break;
                        },
                    }
                }
        ...
        info!(?listener_addr, "Discovered Arion listener address");
    ...
    pub async fn spawn_with_fixed_port(
        ...
        info!(?config_path, ?listener_name, port, "Spawning Arion instance with fixed port");
        ...
        info!(?listener_addr, "Arion listener ready on fixed port");
    ...
    async fn spawn_internal(
        ...
        info!(?config_path, ?listener_addr, "Spawning Arion instance");
    ...
    /// Polls Arion's admin /ready endpoint until it returns 200 (`ProxyState::Live` is set).
    ///
    ...
    /// before Arion in tests, so by the time this returns, the upstream is also ready.
    ///
            ...
            if start.elapsed() > timeout {
                if let Some(ref mut process) = self.process {
                    process.kill().unwrap_or_else(|e| {
                        error!("Failed to kill arion process after timeout: {e}");
                    });
                }
                return Err(Error::ReadyTimeout(timeout));
            }
    ...
    fn drop(&mut self) {
        ...
        if let Some(mut process) = self.process.take() {
            if let Err(e) = process.kill() {
                ...
                if e.kind() != std::io::ErrorKind::InvalidInput {
                    error!("Failed to send kill signal to arion process: {e}");
                }
            }
            ...
            loop {
                match process.try_wait() {
                    ...
                    Ok(None) => {
                        error!(
                            "Arion process did not exit within 5s after kill — \
                             port may remain bound and affect subsequent tests"
                        );
                        break;
                    },
                    Err(e) => {
                        error!("Failed to wait for arion process exit: {e}");
                        break;
                    },
                }
            }
...
fn find_arion_binary() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("ARION_BIN") {
    ...
    let debug_path = workspace_root.join("target/debug/arion");
    if debug_path.exists() {
        return Ok(debug_path);
    }

    let release_path = workspace_root.join("target/release/arion");
    if release_path.exists() {
        return Ok(release_path);
    }

    Err(Error::ProcessStartFailed("Could not find arion binary. Run `cargo build -p arion-proxy` first.".to_owned()))
}
