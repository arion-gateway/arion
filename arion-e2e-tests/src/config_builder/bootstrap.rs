...
use arion_configuration::config::metrics::MetricsConfig;
...
impl BootstrapBuilder {
    ...
    pub fn build_to_temp(&self) -> Result<PathBuf> {
        ...
        let filename = format!("arion-test-{}-{}.yaml", std::process::id(), counter);
    ...
    fn build_bootstrap(&self) -> Result<ArionConfig> {
