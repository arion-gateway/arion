...
pub enum Error {
    ...
    #[error("Failed to start Arion process: {0}")]
    ProcessStartFailed(String),

    #[error("Arion process exited unexpectedly with code: {0:?}")]
    ProcessExitedUnexpectedly(Option<i32>),

    #[error("Arion startup failed (exit code: {exit_code:?}):\n{output}")]
    StartupFailed { exit_code: Option<i32>, output: String },

    #[error("Arion process did not become ready within {0:?}")]
