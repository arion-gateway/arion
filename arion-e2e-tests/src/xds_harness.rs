...
pub struct XdsHarnessOptions {
    ...
    pub arion_options: SpawnOptions,
}

impl Default for XdsHarnessOptions {
    fn default() -> Self {
        Self { timeouts: HarnessTimeouts::default(), log_level: "debug".into(), arion_options: SpawnOptions::default() }
    }
}
...
impl XdsEnabledHarness {
    ...
    pub async fn push_mcp_tool(&self, resource_id: &str, tool: &ArionTool) -> Result<(), HarnessError> {
    ...
    #[must_use]
    pub fn arion(&self) -> &ArionInstance {
        &self.arion
    }

    #[must_use]
    pub fn arion_mut(&mut self) -> &mut ArionInstance {
        &mut self.arion
    }
