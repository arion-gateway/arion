...
use arion_e2e_tests::{ArionInstance, SpawnOptions, TcpTestBackend, TcpTestClient};
...
// false if Arion closed it (EOF or error).
...
async fn setup(backend: &TcpTestBackend) -> (ArionInstance, TcpTestClient) {
    ...
    #[allow(clippy::unwrap_used)]
    let arion = ArionInstance::spawn_auto_port(&config_path, "tcp", SpawnOptions::default()).await.unwrap();
    #[allow(clippy::unwrap_used)]
    let client = TcpTestClient::new(arion.listener_addr().unwrap());
    (arion, client)
}
...
async fn test_connection_limit_enforced() {
    ...
    let (arion, client) = setup(&backend).await;
