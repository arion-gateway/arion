...
async fn test_access_log_listener_all_operators() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-listener-{}.txt", std::process::id()));
    ...
    // Connect to Arion, read backend's greeting, send client message, and close
    {
...
async fn test_access_log_listener_upstream_down() {
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-listener-down-{}.txt", std::process::id()));
    ...
    // Connect to Arion. The upstream is down, so Arion will close the connection.
    {
        let mut stream = TcpStream::connect(listener_addr).await.expect("Failed to connect");
