...
async fn test_access_log_hcm_all_operators() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-hcm-{}.txt", std::process::id()));
    ...
    // Request and Response header bytes for origin-form
    // Note: RESPONSE_HEADERS_BYTES in Arion measures only the size of the header key-value pairs,
    ...
    // Note: Arion normalizes the absolute-form URI to origin-form (removing "http://localhost" = 16 bytes)
...
async fn test_access_log_hcm_upstream_down() {
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-hcm-down-{}.txt", std::process::id()));
...
async fn test_access_log_hcm_original_path_header() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-hcm-origpath-{}.txt", std::process::id()));
...
async fn test_access_log_hcm_backend_500() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-hcm-500-{}.txt", std::process::id()));
...
async fn test_access_log_hcm_multiple_connections() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-hcm-multi-{}.txt", std::process::id()));
