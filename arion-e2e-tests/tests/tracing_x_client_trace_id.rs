...
/// Spins up Arion with tracing enabled at the given sampling rates.
...
// No existing trace context → Arion creates a new root trace.
...
async fn test_client_trace_id_sampling_100_triggers_tracing() {
    let (arion, client, mut backend, config_path) = setup_with_sampling(Some(100), Some(100), Some(100)).await;
...
async fn test_client_trace_id_with_traceparent() {
    ...
    // If Arion incorrectly used X-Client-Trace-Id as the trace_id, the
