...
/// carried by the `x-arion-metadata` header (configured via `incoming_request_header`).
...
async fn test_access_log_custom_operator_incoming_request() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-custom-{}.txt", std::process::id()));
    ...
    let access_log_config = AccessLogConfig {
        blocking: true,
        incoming_request_header: Some(HeaderName::from_static("x-arion-metadata")),
    ...
    // Custom operators extracted from the x-arion-metadata header JSON
...
async fn test_access_log_custom_operator_not_configured_is_graceful() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-custom-noop-{}.txt", std::process::id()));
...
async fn test_access_log_custom_operator_multiple_fields() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-custom-multi-{}.txt", std::process::id()));
