...
async fn test_tls_handshake_metric() {
    ...
    // Spawn Arion tracking the "https" listener address
    let mut arion = ArionInstance::spawn_auto_port(&config_path, "https", SpawnOptions::default())
        .await
        .expect("Failed to spawn Arion");

    // Wait for the cleartext HTTP listener to be ready
    arion.wait_for_listener_at(http_addr, Duration::from_secs(10)).await.expect("HTTP listener not ready");
    ...
    let tls_client = TlsTestClientBuilder::new(arion.listener_addr().unwrap())
