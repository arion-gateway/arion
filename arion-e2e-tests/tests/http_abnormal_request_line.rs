...
async fn test_tc0101_unregistered_method() {
    ...
    // Arion treats unknown methods as opaque tokens and forwards upstream.
...
async fn test_tc0102_method_case_sensitivity() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    // Arion preserves method case and forwards "get" as a token.
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    resp.assert_status(200);
...
async fn test_tc0205_path_traversal() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    // Arion forwards the raw path; normalization is the upstream's responsibility.
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    resp.assert_status(200);
...
async fn test_tc0206_fragment_in_uri() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion forwards request-targets containing a fragment (RFC 7230 §5.3.1 disallows, but not enforced).
    resp.assert_status(200);
...
async fn test_tc0207_double_encoding() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion does not decode the URI; double-encoded sequences pass through to the upstream.
    resp.assert_status(200);
...
async fn test_tc0208_absolute_uri() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion accepts absolute-form request-targets (RFC 7230 §5.3.2).
    resp.assert_status(200);
...
async fn test_tc0303_unsupported_version() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    // RFC 9110 §15.6.6 suggests 505 as more specific; Arion returns generic 400.
    assert_rejected(&response, &[400]);
...
async fn test_tc0403_lf_only_line_ending() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    // Arion tolerates LF-only line endings (RFC 7230 §3.5 "bare LF" SHOULD be accepted).
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    resp.assert_status(200);
...
async fn test_tc0408_leading_blank_lines() {
    ...
    let response = tcp_client.send(&data).await.expect("Failed to send");
    // Arion tolerates leading blank lines as RFC 7230 §3.5 allows.
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    resp.assert_status(200);
