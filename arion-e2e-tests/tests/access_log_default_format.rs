...
async fn test_access_log_default_format_basic() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-default-{}.txt", std::process::id()));
...
async fn test_access_log_default_format_with_original_path() {
    ...
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-default-op-{}.txt", std::process::id()));
