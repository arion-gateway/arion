...
async fn test_http_health_check_excludes_unhealthy() {
    ...
    // stably excluded — guards against the race where Arion received the health-check result
...
async fn test_grpc_health_check_recovery() {
    ...
    // to guard against the race where Arion has received the health-check but hasn't yet
