...
/// Spins up Arion with tracing enabled at 100% sampling.
async fn setup_tracing() -> (ArionInstance, TestClient, TestBackend, std::path::PathBuf) {
...
async fn test_b3_single_basic() {
    let (arion, client, mut backend, config_path) = setup_tracing().await;
...
// Incoming context already has a parent. Arion should generate a new
...
// Arion should accept and spawn a child span.
#[tokio::test]
#[ignore]
#[allow(clippy::indexing_slicing)]
