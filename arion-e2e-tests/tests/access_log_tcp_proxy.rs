...
use arion_configuration::config::log::AccessLogConfig;
...
async fn test_access_log_tcp_proxy_all_operators() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-tcp-{}.txt", std::process::id()));
    ...
    let arion = ArionInstance::spawn_auto_port(&config_path, "tcp", SpawnOptions::default())
        .await
        .expect("Failed to spawn Arion");

    let listener_addr = arion.listener_addr().expect("Missing listener address");

    // Connect to Arion, read backend's greeting, send client message, and close connection
    {
...
async fn test_access_log_tcp_proxy_upstream_down() {
    // Create a temporary file for the access log
    let log_dir = std::env::temp_dir();
    let log_path = log_dir.join(format!("arion-test-access-log-tcp-down-{}.txt", std::process::id()));
    ...
    let arion = ArionInstance::spawn_auto_port(&config_path, "tcp", options).await.expect("Failed to spawn Arion");

    let listener_addr = arion.listener_addr().expect("Missing listener address");

    // Connect to Arion and send some data. Arion will try to connect to the upstream,
    // fail, and close the connection.
    {
        let mut stream = TcpStream::connect(listener_addr).await.expect("Failed to connect");
        let _ = stream.write_all(b"Hello?").await;
        let mut buf = [0u8; 128];
        // Expect connection to be closed by Arion
        let n = stream.read(&mut buf).await.unwrap_or(0);
        assert_eq!(n, 0, "Connection should be closed by Arion")
    };

    // Wait briefly for the access log to be written and Arion to flush debug logs
