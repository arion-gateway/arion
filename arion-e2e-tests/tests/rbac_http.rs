...
async fn test_http_rbac_allow_header_permission_match() {
    ...
    let client = TestClient::new(arion.listener_addr().unwrap()).with_header("x-custom", "allowed");
    let response = client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);
...
async fn test_http_rbac_allow_host_header() {
    ...
    let client = TestClient::new(arion.listener_addr().unwrap()).with_header("host", "allowed.example.com");
    let response = client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);
...
async fn test_http_rbac_deny_header_blocks() {
    ...
    let client = TestClient::new(arion.listener_addr().unwrap()).with_header("x-blocked", "true");
    let response = client.get("/test").await.unwrap();
    response.assert_status(StatusCode::FORBIDDEN);
...
async fn test_http_rbac_principal_header_match() {
    ...
    let client = TestClient::new(arion.listener_addr().unwrap()).with_header("authorization", "Bearer valid-token");
    let response = client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);
...
async fn test_http_rbac_principal_header_no_match() {
    ...
    let client = TestClient::new(arion.listener_addr().unwrap()).with_header("authorization", "Bearer invalid-token");
    let response = client.get("/test").await.unwrap();
    response.assert_status(StatusCode::FORBIDDEN);
...
async fn test_http_rbac_multiple_policies() {
    ...
    let admin_client = TestClient::new(arion.listener_addr().unwrap()).with_header("x-role", "admin");
    let response = admin_client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let user_client = TestClient::new(arion.listener_addr().unwrap()).with_header("x-role", "user");
    let response = user_client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let guest_client = TestClient::new(arion.listener_addr().unwrap()).with_header("x-role", "guest");
...
async fn test_http_rbac_multiple_permissions_or() {
    ...
    let client1 = TestClient::new(arion.listener_addr().unwrap()).with_header("x-api-key", "key1");
    let response = client1.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let client2 = TestClient::new(arion.listener_addr().unwrap()).with_header("x-api-key", "key2");
    let response = client2.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let client3 = TestClient::new(arion.listener_addr().unwrap()).with_header("x-api-key", "key3");
...
async fn test_http_rbac_permission_and_principal() {
    ...
    let both_client = TestClient::new(arion.listener_addr().unwrap())
        .with_header("x-service", "allowed-service")
        .with_header("x-user", "authorized-user");
    let response = both_client.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let service_only = TestClient::new(arion.listener_addr().unwrap()).with_header("x-service", "allowed-service");
    let response = service_only.get("/test").await.unwrap();
    response.assert_status(StatusCode::FORBIDDEN);

    let user_only = TestClient::new(arion.listener_addr().unwrap()).with_header("x-user", "authorized-user");
...
async fn test_http_rbac_header_present() {
    ...
    let with_auth = TestClient::new(arion.listener_addr().unwrap()).with_header("authorization", "any-value");
    let response = with_auth.get("/test").await.unwrap();
    response.assert_status(StatusCode::OK);

    let without_auth = TestClient::new(arion.listener_addr().unwrap());
