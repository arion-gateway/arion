...
use arion_e2e_tests::{ArionInstance, PreConfiguredResponse, RequestBuilder, SpawnOptions, TestBackend, TestClient};
use http::StatusCode;
...
async fn test_route_match_collision_bug() {
        ...
        .listener(
            ListenerBuilder::new("http").port(0).filter_chain(
                FilterChainBuilder::new("main").hcm(
                    HcmBuilder::new().http1().local_rate_limit(hcm_rate_limit).route_config(
                        RouteConfigBuilder::new("routes")
                            .virtual_host(
                                VirtualHostBuilder::new("vhost-1").domains(["api.example.com"]).route(
                                    RouteBuilder::new()
                                        .match_prefix("/")
                                        .cluster("backend")
                                        .local_rate_limit_override(restrictive_override),
                                ),
                            )
                            .virtual_host(
                                VirtualHostBuilder::new("vhost-2").domains(["admin.example.com"]).route(
                                    RouteBuilder::new()
                                        .match_prefix("/")
                                        .cluster("backend")
                                        .local_rate_limit_override(permissive_override),
                                ),
                            ),
                    ),
                ),
    ...
    let response1 =
        client.send(RequestBuilder::get("/test").header("host", "admin.example.com")).await.expect("request 1");
    ...
    let response2 =
        client.send(RequestBuilder::get("/test").header("host", "admin.example.com")).await.expect("request 2");
