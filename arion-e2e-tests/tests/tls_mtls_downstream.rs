...
use arion_e2e_tests::config_builder::{presets, BootstrapBuilder, ClusterBuilder, DownstreamTlsBuilder};
use arion_e2e_tests::{
...
async fn test_mtls_downstream_different_valid_certs() {
    ...
    let client1 = TlsTestClientBuilder::new(arion.listener_addr().unwrap())
        .server_name("dublin.beefcake.example.com")
        .root_ca(certs.beefcake_ca_chain())
        .client_cert(certs.beefcake_athlone_cert(), certs.beefcake_athlone_key())
        .build()
    ...
    let client2 = TlsTestClientBuilder::new(arion.listener_addr().unwrap())
        .server_name("dublin.beefcake.example.com")
        .root_ca(certs.beefcake_ca_chain())
