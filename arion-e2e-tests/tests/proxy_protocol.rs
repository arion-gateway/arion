...
async fn pp_v2_passes_original_source_to_backend() {
    ...
    let arion = ArionInstance::spawn_auto_port(&config_path, "http", SpawnOptions::default()).await.expect("spawn");

    let mut stream = ProxyProtocolTcpClient::v2(claimed_src(), claimed_dst())
        .connect(arion.listener_addr().unwrap())
        .await
        .expect("connect");
...
async fn pp_required_but_missing_rejects_connection() {
    ...
    let mut stream =
        ProxyProtocolTcpClient::no_header().connect(arion.listener_addr().unwrap()).await.expect("connect");
...
async fn pp_v2_tlv_pass_through_to_upstream() {
    ...
    let config_path = bootstrap.build_to_temp().expect("config");
    let arion = ArionInstance::spawn_auto_port(&config_path, "tcp", SpawnOptions::default()).await.expect("spawn");
...
async fn pp_v2_with_tls_inspector_routes_by_sni() {
    ...
    let config_path = bootstrap.build_to_temp().expect("config");
    let arion = ArionInstance::spawn_auto_port(&config_path, "https", SpawnOptions::default()).await.expect("spawn");
...
async fn cluster_emits_pp_v2_to_plain_tcp_upstream() {
    ...
    let mut stream = tokio::net::TcpStream::connect(arion.listener_addr().unwrap()).await.expect("connect");
...
async fn internal_listener_basic_chain() {
    ...
    let config_path = bootstrap.build_to_temp().expect("config");
    let arion = ArionInstance::spawn_auto_port(&config_path, "outer", SpawnOptions::default()).await.expect("spawn");
