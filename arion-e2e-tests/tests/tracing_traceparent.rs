...
async fn setup() -> (ArionInstance, TestClient, TestBackend, std::path::PathBuf) {
...
async fn test_traceparent_basic() {
    let (arion, client, mut backend, config_path) = setup().await;
...
// Arion may pass it through or generate a new traceparent (root span).
#[tokio::test]
#[ignore]
