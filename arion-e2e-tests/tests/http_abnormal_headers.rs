...
async fn test_tc0510_obsolete_line_folding() {
    ...
    // Arion rejects obsolete line folding as RFC 7230 §3.2.4 recommends.
...
async fn test_tc0603_invalid_host_port() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion does not validate the authority component's port; the request is forwarded.
...
async fn test_tc0604_empty_host_value() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion routes the empty-Host request but the upstream connection fails → 502.
...
async fn test_tc0608_cl_and_te_coexist() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion prioritizes Transfer-Encoding over Content-Length and forwards.
...
async fn test_tc0610_stacked_te_values() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion accepts stacked "chunked, chunked" TE and forwards.
    resp.assert_status(200);
