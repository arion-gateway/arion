...
async fn test_allowed_connections_reach_backend() {
    ...
    backend.set_default_response(arion_e2e_tests::PreConfiguredResponse::with_body("ok")).await;
...
async fn test_over_limit_drops_tcp_connection() {
    ...
    let tcp = TcpTestClient::new(arion.listener_addr().unwrap());
...
async fn test_quota_caching_limits_rls_calls() {
    ...
    let config_path = bootstrap.build_to_temp().unwrap();
    let arion = ArionInstance::spawn_auto_port(&config_path, "http", SpawnOptions::default()).await.unwrap();
    let arion_addr = arion.listener_addr().unwrap();

    for _ in 0..6 {
        let client = TestClient::new(arion_addr);
        let resp = client.get("/").await.unwrap();
        resp.assert_status(StatusCode::OK);
    }
