...
async fn test_circuit_breaker_max_requests_overflow() {
    ...
    let client = Arc::new(TestClient::new(arion.listener_addr().unwrap()));
...
async fn test_circuit_breaker_high_priority_independent() {
    ...
    let high_route = RouteBuilder::new().match_prefix("/high").cluster("backend").with_proto(|r| {
        if let Some(arion_data_plane_api::envoy_data_plane_api::envoy::config::route::v3::route::Action::Route(
            ref mut ra,
        )) = r.action
        {
            ra.priority = 1; // HIGH
