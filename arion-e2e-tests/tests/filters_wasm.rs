...
use arion_e2e_tests::{
    ArionInstance, PreConfiguredResponse, RequestBuilder, SpawnOptions, TestBackend, TestCerts, TestClient,
    TlsTestClientBuilder,
};
use http::StatusCode;

async fn setup_wasm(wasm_builder: WasmBuilder) -> (TestBackend, ArionInstance, TestClient, PathBuf) {
    ...
    (backend, arion, client, config_path)
}
...
async fn test_wasm_dummy_filter_missing_authorization() {
    let (_backend, _arion, client, _cfg) = setup_wasm(dummy_filter_builder()).await;
...
async fn test_wasm_dummy_filter_authorized() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(dummy_filter_builder()).await;
...
async fn test_wasm_header_mutations_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(header_mutations_filter_builder()).await;
...
async fn test_wasm_header_api_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(header_api_filter_builder()).await;
...
async fn test_wasm_headers_map_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(headers_map_filter_builder()).await;
...
async fn test_wasm_body_mutation_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(body_mutation_filter_builder()).await;
    backend.set_default_response(PreConfiguredResponse::with_body("backend response")).await;
...
async fn test_wasm_callout_body_filter() {
    ...
    let arion_instance =
        ArionInstance::spawn_auto_port(&config_path, "http", SpawnOptions::default()).await.expect("spawn arion");
    #[allow(clippy::unwrap_used)]
    let client = TestClient::new(arion_instance.listener_addr().unwrap());
...
async fn test_wasm_metadata_filter() {
    ...
    let (mut backend, _arion, client, _cfg) = setup_wasm(builder).await;
    backend.set_default_response(PreConfiguredResponse::with_body("backend response")).await;
...
async fn test_wasm_metadata_filter_sni() {
    ...
    let client = TlsTestClientBuilder::new(arion.listener_addr().unwrap())
...
async fn test_wasm_config_logger_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(config_logger_filter_builder()).await;
    backend.set_default_response(PreConfiguredResponse::with_body("backend response")).await;
...
async fn test_wasm_access_log_operator_filter() {
    ...
    let log_path = log_dir.join(format!("arion-test-access-log-wasm-{}.txt", std::process::id()));
    ...
    let bootstrap = BootstrapBuilder::new()
        ...
        .access_log(arion_configuration::config::log::AccessLogConfig {
            blocking: true,
            ...
            ..arion_configuration::config::log::AccessLogConfig::default()
        });
    ...
    let raw_req = arion_e2e_tests::RawHttpRequestBuilder::new()
        .method("GET")
        .uri("/")
        .host("localhost")
...
use arion_configuration::config::metrics::{CustomMetric, CustomMetrics, MetricsConfig};
use arion_e2e_tests::PortBlock;
use http::HeaderName;
...
async fn test_wasm_shared_atomic_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(shared_atomic_filter_builder()).await;
...
async fn test_wasm_shared_blob_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(shared_blob_filter_builder()).await;
...
async fn test_wasm_grpc_callout_filter() {
    ...
    let bootstrap = BootstrapBuilder::new()
        ...
        .cluster(arion_e2e_tests::config_builder::presets::ext_proc_cluster("service", callout_backend.addr()));
...
async fn test_wasm_direct_response_filter() {
    let (backend, _arion, client, _cfg) = setup_wasm(direct_response_filter_builder()).await;
    backend.set_default_response(PreConfiguredResponse::with_body("backend response")).await;
...
async fn test_wasm_uri_status_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(uri_status_filter_builder()).await;
...
async fn test_wasm_materialize_filter() {
    let (mut backend, _arion, client, _cfg) = setup_wasm(materialize_filter_builder()).await;
