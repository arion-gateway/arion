...
fn jwt_authn_for_tests() -> JwtAuthentication {
    let provider = JwtProvider {
        ...
        // Required so that arion inserts JwtClaims into request extensions, making them
...
async fn spawn_jwt_cedar_instance(backend: &TestBackend) -> ArionInstance {
    ...
    let config_path = bootstrap.build_to_temp().unwrap();
    ArionInstance::spawn_auto_port(&config_path, "http", SpawnOptions::default()).await.unwrap()
}
...
async fn test_cedar_jwt_frontend_get_allowed() {
    ...
    let arion = spawn_jwt_cedar_instance(&backend).await;
    let token = make_jwt("svc-frontend");
    let client =
        TestClient::new(arion.listener_addr().unwrap()).with_header("authorization", format!("Bearer {token}"));
...
/// reuse) and many of these are fired concurrently across multiple Arion
