...
use arion_e2e_tests::{
    cleanup_config_file, ArionInstance, PreConfiguredResponse, RawHttpRequestBuilder, RawHttpResponse, SpawnOptions,
...
async fn test_tc1206_url_encoding_bypass() {
    ...
    let response = tcp_client.send(&req).await.expect("Failed to send");
    let resp = RawHttpResponse::parse(&response).expect("Expected a response");
    // Arion forwards the raw percent-encoded path; normalization is the upstream's responsibility.
    resp.assert_status(200);
