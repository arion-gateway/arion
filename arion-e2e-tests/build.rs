...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.args(["build", "--target", "wasm32-unknown-unknown", "--workspace"]).current_dir("../arion-wasm-sdk");
    ...
    if !status.success() {
        println!("cargo:warning=Failed to build Wasm examples in arion-wasm-sdk! Wasm E2E tests may fail.");
    }
