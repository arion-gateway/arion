# Arion E2E Test Harness

End-to-end testing utilities for Arion Proxy.

## Quick Start

```bash
# Build the proxy binary first
cargo build --all-features -p arion-proxy

# Run e2e tests (ignored by default)
# We recommend using a single thread for e2e tests to avoid race conditions
cargo test -p arion-e2e-tests -- --ignored --test-threads=1
```
...
```
                                                    ┌──────────────┐
TestClient  ──────►  ArionInstance  ──────────────► │ TestBackend  │
   ...
   (generates YAML)               (manages xDS server + Arion)
```

**Static mode**: Generate YAML config with `BootstrapBuilder`, write to file, spawn Arion.

**Dynamic mode**: Use `XdsEnabledHarness` which handles xDS server setup and Arion lifecycle.
...
use arion_e2e_tests::{ArionInstance, PreConfiguredResponse, SpawnOptions, TestBackend, TestClient};
...
async fn test_basic_proxy() {
    ...
    // spawn_auto_port discovers the actual port from Arion's logs
...
Use `XdsEnabledHarness` for dynamic configuration via xDS. It handles xDS server setup, Arion lifecycle, and ACK/NACK tracking automatically:
...
async fn test_dynamic_xds_config() {
    ...
    // Start harness (spawns xDS server and Arion, waits for connection)
...
```rust
use std::time::Duration;
use arion_e2e_tests::config_builder::xds::PushResult;
...
```rust
use std::time::Duration;
use arion_e2e_tests::{ArionInstance, SpawnOptions};

// Recommended: Use port 0 in config and discover port from logs
let arion = ArionInstance::spawn_auto_port(&config_path, "http", SpawnOptions::default()).await?;
...
let options = SpawnOptions::default()
    ...
    .with_num_cpus(4)
    .with_cleanup();
let arion = ArionInstance::spawn_auto_port(&config_path, "http", options).await?;

// For xDS tests where no listener is configured initially
let arion = ArionInstance::spawn_no_listener(&config_path, SpawnOptions::default()).await?;
...
All-in-one harness for xDS-based testing. Manages xDS server, Arion instance, and configuration pushing.

```rust
use std::time::Duration;
use arion_e2e_tests::{XdsEnabledHarness, XdsHarnessOptions, HarnessTimeouts};
...
// Access Arion instance for listener readiness checks
harness.arion_mut().wait_for_listener_at(addr, Duration::from_secs(10)).await?;
...
| `HarnessError::Nack` | xDS configuration rejected by Arion |
...
4. Always call `shutdown()` on ArionInstance or XdsEnabledHarness
5. Verify both the response and what the backend received
6. Build the proxy binary first: `cargo build -p arion-proxy`
...
async fn test_mcp_gateway_tools_list() {
    ...
    let mut client = McpTestClient::new(
        format!("http://{}", arion.listener_addr().unwrap())
    );
