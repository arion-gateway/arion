...
#[command(about = "Run an interactive Arion MCP Gateway semantic-search demo backed by built-in BM25 ranking.")]
struct Args {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let arion = ArionInstance::spawn_auto_port(&config_path, LISTENER_NAME, spawn_options).await?;
    let listener_addr = arion
        .listener_addr()
        .ok_or_else(|| io::Error::new(io::ErrorKind::AddrNotAvailable, "Arion did not report a listener address"))?;
    ...
    println!();
    println!("Press Ctrl-C to stop Arion and the demo backends.");
...
fn build_config(args: &Args, backends: &DemoBackends) -> arion_e2e_tests::Result<PathBuf> {
    let gateway = McpGatewayBuilder::new("arion-mcp-local-demo", "1.0.0")
        .tools(demo_tools())
...
impl DemoBackends {
    async fn start() -> arion_e2e_tests::Result<Self> {
...
fn demo_tools() -> Vec<ArionMcpTool> {
    vec![
...
fn print_startup(args: &Args, backends: &DemoBackends, config_path: &std::path::Path, listener_addr: SocketAddr) {
    println!("Arion MCP semantic-search demo is running");
