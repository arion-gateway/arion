...
//! Shared ABI types between the Arion proxy (host) and the Arion Wasm SDK (guest).
//!
...
/// Error returned when converting a raw `i32` into [`ArionWasmError`] and the
/// value does not match any known variant.
/// Idiomatic Rust error type for Arion SDK.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ArionWasmError {
    NotFound,
    BufferTooSmall,
    InvalidMemoryAccess,
    InternalError,
...
impl core::fmt::Display for ArionWasmError {
...
impl std::error::Error for ArionWasmError {}

/// Action codes that a plugin returns to the host to control filter flow.
///
/// Mirrored on the host side by
/// `arion-lib/src/listeners/http_connection_manager/wasm/types.rs` (which
...
impl From<ArionWasmError> for i32 {
    #[inline]
    fn from(e: ArionWasmError) -> Self {
        match e {
            ArionWasmError::NotFound => 1,
            ArionWasmError::BufferTooSmall => 2,
            ArionWasmError::InvalidMemoryAccess => 3,
            ArionWasmError::InternalError => 4,
            ArionWasmError::Timeout => 5,
        }
    }
}
