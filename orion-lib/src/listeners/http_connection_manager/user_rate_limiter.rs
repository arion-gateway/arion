...
impl UserRateLimiter {
    ...
    pub async fn apply_request(&mut self, request: &mut http::Request<OrionRequestBody>) -> FilterDecision {
            ...
            tokio::spawn(async {
                loop {
                    ...
                    USER_RATE_LIMITERS.retain(|usr, tb| {
                        ...
                        if !res {
                            debug!(target: "user_rate_limiter", "cleaning up token bucket for user_id: {usr}");
                        }
                        res
                    })
                }
        ...
        let Some(user_id) = request.headers().get(self.inner.user_id_header.as_str()).and_then(|v| v.to_str().ok())
        else {
            debug!(target: "user_rate_limiter", "no user_id found in request headers");
            return FilterDecision::Continue;
        };

        // Try to look up the TokenBucket for the given user_id in the global USER_RATE_LIMITERS map.
        // If found, return a reference to the corresponding token bucket.
        // Otherwise, look up the user_id configuration in the filter and create a new TokenBucket.
        // If no configuration is found, or not configured, simply return Continue.
        //

        // Happy-path: an entry for the user_id already exists in the global map...
        //
        if let Some(tb) = USER_RATE_LIMITERS.get(user_id) {
            if tb.consume(1) {
                debug!(target: "user_rate_limiter", "consumed token for user_id: {user_id}");
                return FilterDecision::Continue;
            } else {
                debug!(target: "user_rate_limiter", "rate limited for user_id: {user_id}");
                return FilterDecision::rate_limited(request.version());
            }
        }

        // If the entry for the user_id does not exist in the global map, let's try to insert a new one.
        //
        let token_bucket = USER_RATE_LIMITERS.entry(user_id.into()).or_try_insert_with(|| {
            let limit = if let Some(limit) = self.inner.user_rate_limits.get(&Some(user_id.into())) {
                limit
            } else if let Some(limit) = self.inner.user_rate_limits.get(&None) {
                limit
            } else {
                // Configuration not found for this user... return Continue
                debug!(target: "user_rate_limiter", "no rate limit found for user_id: {user_id}");
            ...
            match limit {
                Limit::LocalRateLimit(l) => {
                    let Some(tb) = &l.token_bucket else {
                        ...
                        debug!(target: "user_rate_limiter", "no token bucket found for user_id: {user_id}");
                        return Err(FilterDecision::Continue);
                    };
