...
use crate::{transport::HttpChannels, OrionRequestBody};

#[cfg(feature = "metrics")]
use {
    crate::{get_shard_id, with_metric},
    opentelemetry::KeyValue,
    orion_metrics::metrics::clusters,
};
...
mod tests {
    ...
    fn request_permit_accounts_for_acquisition_lifetime() {
        ...
        let request = Request::builder().uri("http://127.0.0.1:18080/tool").body(OrionRequestBody::default()).unwrap();
    ...
    fn authority_routing_validates_and_accepts_request_authority() {
        ...
        let cluster = clusters_manager::add_cluster(build_original_dst_cluster(cluster_name, 1)).unwrap();
        let circuit_breaker = cluster.circuit_breaker().unwrap();
        let request = Request::builder().uri("/tool").body(OrionRequestBody::default()).unwrap();
        let specifier = ClusterSpecifier::Cluster(cluster_name.into());
        ...
        let request =
            Request::builder().uri("/tool").header(HOST, "127.0.0.1:18080").body(OrionRequestBody::default()).unwrap();
        let acquired = acquire_http_upstream(
            &specifier,
            &request,
            &[],
