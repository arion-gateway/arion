...
use orion_e2e_tests::{
    cleanup_config_file, generate_jwt_token, JwtKeyPair, OrionInstance, PreConfiguredResponse, RequestBuilder,
...
impl JwtRerouteHarness {
    async fn start(clear_route_cache: bool) -> Self {
        ...
        let client = TestClient::new(orion.listener_addr().expect("listener addr"));

        Self { admin, fallback, orion, client, jwt_keys, config_path }
    }
