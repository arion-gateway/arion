...
use orion_e2e_tests::{
    cleanup_config_file, parse_metric_value, OrionInstance, PortBlock, SpawnOptions,
    TcpTestBackend, TestClient,
};
...
async fn test_tcp_proxy_metrics() {
    ...
    let listener = ListenerBuilder::new("tcp")
        ...
        .filter_chain(
            FilterChainBuilder::new("main")
                .tcp_proxy(TcpProxyBuilder::new("tcp_stats").cluster("backend"))
        );
    ...
    // 2. Open a TCP connection and keep it open to verify active connection metric
    {
        ...
        assert_eq!(cx_active, 1.0, "Expected exactly 1 active TCP connection");
    ...
    for _ in 0..20 {
        ...
        cx_active = parse_metric_value(metrics, "tcp_downstream_cx_active").unwrap_or(0.0);
        if cx_active == 0.0 {
            break;
        }
    ...
    assert_eq!(cx_active, 0.0, "Expected active TCP connections to drop to 0");
    ...
    let cx_length_count = parse_metric_value(metrics, "tcp_downstream_cx_length_ms_count").expect("Missing cx_length_ms_count metric");
    ...
    assert_eq!(cx_total, 2.0, "Expected exactly 2 total downstream TCP connections");
    assert_eq!(cx_destroy, 2.0, "Expected exactly 2 destroyed downstream TCP connections");
    assert_eq!(cx_length_count, 2.0, "Expected exactly 2 recorded connection lengths");

    let expected_rx = (b"ping".len() * 2) as f64;
    let expected_tx = (b"hello".len() * 2) as f64;
