...
async fn test_force_trace_no_context() {
    let (orion, client, mut backend, config_path) = setup(Some(100), Some(100), Some(100)).await;
...
async fn test_force_trace_overrides_0_sampling() {
    let (orion, client, mut backend, config_path) = setup(Some(0), Some(0), Some(0)).await;
