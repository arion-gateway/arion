...
This directory contains runnable examples for Orion MCP Gateway semantic search.

- `mcp_semantic_search.rs` starts Orion, four local REST backends, and a generated MCP Gateway config using built-in BM25 ranking.
...
The MCP gateway can inject a synthetic tool named `semantic_search`. When a client calls it with a `user_query`, Orion ranks available tools using each tool's name, description, and input schema metadata.

By default, ranking uses BM25 locally and does not require an embeddings service. If `semantic_search_tool.embeddings` is configured, Orion can use remote vector ranking and falls back to BM25 if query embedding fails or any candidate lacks a vector.
...
```bash
cargo run -p orion-e2e-tests --features fastembed-service --example fastembed_embedding_service
```
